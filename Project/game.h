...
class Game {
    ...
    friend class Scene;
public: 
    Game(HWND hwnd, int gamemode);//gamemod为1是斗地主，2是干瞪眼
    ~Game();
...
private:
    ...
    int times=1; //本局的倍率
    int basispoint;//本局的基本分，取决于叫地主的点数
    Player* ptr,* DiZhu, *pptr;//地主指针和其余的两个指针
    int DiZhuCard[8]; //8张地主牌
    Cards Paidui; //斗地主是两副牌，干瞪眼是一副牌
    HWND hMainWnd;
    Scene* scene;
    int callscore[4]; //叫地主的点数
    int calltimes; //有几个人进行了叫地主的操作
